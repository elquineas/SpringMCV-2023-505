...
import com.elquineas.address.dao.AddrDao;
import com.elquineas.address.dao.BuyerDao;
import com.elquineas.address.dao.ScoreDao;
...
public class AddrServiceImplV1 implements AddrService{
	 ...
	 * Java의 전통적인 객체생성 코드와 대비되는 Spring Framework Project의 독특한 기법 
	 * 객체를 선언만 하고, 의존성 주입 절차를 수행하면 
	 * Spring Container에 의해 객체가 이미 생성되어 준비된 객체를 주입해주는 기능
	 * 
	 * 의존성 주입하는 방법은
	 * 필드주입, 생성자주입, Setter주입 이 있다.
	 */
	 ...
	 * 다시 객체를 선언해야하는 문제가 발생한다
	 * 
	 * 참고로 메모리 누수정도 
	 * public > protected > private 순으로 작아진다
	 */
	@Autowired
	protected BuyerDao buyerDao;
	
	// 생성자 의존성 주입 (Constructor Dependency Inject)
	/*
	 * 생성자를 통해서 주입될 객체는 final 로 선언을 한다
	 * 이 방식의 특징은 한번 주입이 되면, 
	 * 코드가 작동되는 중에 또다시 주입을 할수 없다. 객체 보호가 완전하다.
	 * 객체의 순환참조가 없다.
	 * 이방식은 메모리 누수가 없고, 객체가 보호되며, 순환참조에의한
	 * 코드의 exception을 막는다
	 * @Autowired 를 붙이지 않아도 된다
	 * 
	 * 이 방식을 사용하려면 반드시 생성자 메서드가 있어야 한다.
	 * 코드가 다소 번잡해 진다
	 * 실수로 생성자 메서드를 만들지 않았을 경우 Nullpoint exceprion이 발생한다.
	 */
	...
	// setter 의존성 주입(Setter Dependency Inject)
	/*
	 * 객체를 필드영역에 선언만 하고 별도의 메서드를 통하여 주입을 하는 방법
	 * 누군가 실수로, 임의로 객체를 변경할수 있다.
	 * method가 public 으로 완전 개방된 상태이기 때문에
	 */
	@Autowired
	...
	public String idCheck(String id) {
		AddrDto aDto = addrDao.findById(id);
		if(aDto == null) {
			return "OK";
		}else {
			return "FAIL";

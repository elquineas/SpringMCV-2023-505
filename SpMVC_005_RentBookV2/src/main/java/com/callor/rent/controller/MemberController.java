...
//		return String.format("redirect:/member/%s/detail", mcode);
	...
	public List<MemberDto> name_search(@RequestParam(name = "m_name", required=false, defaultValue="-1") String mName, Model model ) {
		if(mName.equals("-1")) {
			return null;
		}
		List<MemberDto> mList = mService.findByMName(mName);
		log.debug("(컨트롤러)리스트 확인 : {}",mList);
		return mList;
	}
